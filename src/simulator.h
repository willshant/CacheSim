...
public:
    ...
    simulator(int nlevels, bool L1_unified, bool L2_unified, std::string L1, std::string L2, float L1_hitTime, float L2_hitTime, float DRAM_accessTime){
        ...
        if(L1_unified){
            ...
            L1Cache.push_back(Cache(L1_A, L1_B, L1_C, L1_strategy, 0));  // right ??
        }else{
            std::stringstream temp;
            temp.str (L1);  //convert string to stringstream
            std::string configuration;
            ...
            L1Cache.push_back(Cache(L1_I_A, L1_I_B, L1_I_C, L1_I_strategy, 0));
            L1Cache.push_back(Cache(L1_D_A, L1_D_B, L1_D_C, L1_D_strategy, 0));
        }
        if(nlevels > 1){
            if(L2_unified){
                std::stringstream temp;
                ...
                //int L1_writeScheme = atoi(L1_configuration[4].c_str());
                L1Cache.push_back(Cache(L2_A, L2_B, L2_C, L2_strategy, 0));
            }else{
                std::stringstream temp;
                temp.str (L2);  //convert string to stringstream
                std::string configuration;
                ...
                L2Cache.push_back(Cache(L2_I_A, L2_I_B, L2_I_C, L2_I_strategy, 0));
                L2Cache.push_back(Cache(L2_D_A, L2_D_B, L2_D_C, L2_D_strategy, 0));
            }
    ...
    void fetch(long address, int level, Cache C){
        ...
        for(int col = 0 ; col < C.associativity ; col ++){
            Block data = C.myCache[(int)index][col];
    ...
    void printResult(){
        ...
        if(nlevels > 1){
            ...
            std::cout << totalDemandFetches_L2 << " demand fetches for L12 cache" << std::endl;
